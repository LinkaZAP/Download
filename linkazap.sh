...
DOWNLOAD_URL="https://downloader.playlinky.com/PlayLinky.zip"
...
wget --no-check-certificate -O "$HOME/$ZIP_FILENAME" "$DOWNLOAD_URL"
...
cd "$INSTALL_DIR"
# Renomeia o executável DENTRO da pasta PlayLinky
mv PlayLinky/linkazap PlayLinky/.linkazap
# Move todos os arquivos para o diretório de instalação principal
mv PlayLinky/* .
...
pm2 start "$INSTALL_DIR/.linkazap" --name "$PM2_PROCESS_NAME"
pm2 startup
pm2 save --force
