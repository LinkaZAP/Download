...
# ETAPA 1: LIMPEZA DE INSTALAÇÃO ANTIGA (Como você pediu)
if [ -d "$INSTALL_DIR" ]; then
    echo -e "${C_YELLOW}Instalação anterior encontrada. Preparando para uma instalação limpa...${C_NC}"
    # Apenas tenta parar o processo se o PM2 estiver instalado
...
# ETAPA 2: VERIFICAÇÃO COMPLETA DE PRÉ-REQUISITOS
...
if [ "$NODE_VERSION_MAJOR" -lt "$REQUIRED_NODE_VERSION" ]; then
    echo -e "${C_RED}ERRO: A versão do seu Node.js é ${NODE_VERSION_MAJOR}, mas a mínima requerida é ${REQUIRED_NODE_VERSION}.${C_NC}"
    echo -e "${C_YELLOW}Por favor, atualize o seu Node.js e tente novamente.${C_NC}"
    exit 1
fi
echo -e "${C_GREEN}Todos os pré-requisitos foram atendidos.${C_NC}"

# ETAPA 3: DOWNLOAD E CONFIGURAÇÃO
echo -e "\n${C_BLUE}[ETAPA 2/3] Baixando e configurando a aplicação...${C_NC}"
...
# ETAPA 4: INICIALIZAÇÃO COM PM2
echo -e "\n${C_BLUE}[ETAPA 3/3] Configurando o serviço de inicialização automática...${C_NC}"
