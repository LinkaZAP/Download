...
# 1. Limpeza de instalação antiga
...
# 2. VERIFICAÇÃO DE PRÉ-REQUISITOS (NODE.JS)
...
if [ "$NODE_VERSION_MAJOR" -lt "$REQUIRED_NODE_VERSION" ]; then
    ...
    echo -e "${C_YELLOW}Dica: Use o NVM (Node Version Manager) para gerenciar as versões facilmente.${C_NC}"
    exit 1
fi
echo -e "${C_GREEN}Node.js v${NODE_VERSION_MAJOR} encontrado. Requisito atendido.${C_NC}"

# 3. Instalação de outras dependências
